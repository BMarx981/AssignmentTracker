...
void main() {
  ...
  test('seeds two students with courses on both sides', () {
    expect(payload.students.map((s) => s.name), ['Felix', 'Nora']);
    final felix = payload.students.first;
    expect(felix.synergy!.courses, hasLength(5));
    expect(felix.canvas!.courses, hasLength(2));
  });

  test('Canvas courses merge into their Synergy counterparts', () {
    final felix = payload.students.first;
    final merged = mergeData(felix.canvas, felix.synergy);
    ...
    expect(math.items.any((i) => i.source == 'both'), isTrue,
        reason: 'name similarity should link Canvas rows to Synergy rows');
    expect(math.items.any((i) => i.source == 'canvas'), isTrue,
    ...
    for (final s in payload.students) {
      for (final c in mergeData(s.canvas, s.synergy)) {
        for (final i in c.items) {
          expect(range.contains(i.date), isTrue,
  ...
  test('the catch-up list is non-empty and led by the failing courses', () {
    ...
    for (final c in mergeData(felix.canvas, felix.synergy)) {
      for (final i in c.items) {
        final score = priorityScore(
          item: i,
          course: c,
          today: today,
          thresholds: felix.scoreThresholds,
          statusByKey: felix.assignmentStatus,
          range: range,
        );
  ...
  test('the score threshold pulls a low graded item into the list', () {
    final felix = payload.students.first;
    expect(felix.scoreThresholds['Language Arts 6'], 70);
    final la = mergeData(felix.canvas, felix.synergy)
    ...
    expect(isActionable(quiz, la.name, felix.scoreThresholds), isTrue);
  ...
  test('seeded local statuses attach to real assignments', () {
    ...
    for (final entry in felix.assignmentStatus.entries) {
      expect(byKey.containsKey(entry.key), isTrue,
    ...
    expect(getLocalStatus(essay, felix.assignmentStatus), isNull);

    // ...while the one on a missing item shows through.
    final varExpr = byKey.values
        .firstWhere((i) => i.name.contains('Write variable expressions'));
    expect(getLocalStatus(varExpr, felix.assignmentStatus)?.status, 'planned');
  });
  ...
  test('seeded comments attach to real assignments', () {
    ...
    expect(felix.comments, isNotEmpty);
    for (final key in felix.comments.keys) {
